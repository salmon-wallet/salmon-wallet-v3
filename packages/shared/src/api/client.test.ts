...
import type { Network, ApiClientConfig } from './client';

// Helper to check if backend is available
async function isBackendAvailable(): Promise<boolean> {
  try {
    const realAxios = await import('axios');
    await realAxios.default.get('http://localhost:3000/health', { timeout: 1000 });
    return true;
  } catch {
    return false;
  }
...
describe('API Client Module', () => {
  ...
  describe('Network Service', () => {
    ...
    beforeEach(async () => {
      ...
      if (!backendAvailable) {
        console.warn('Backend not available, using mocks');
      }
    });
    ...
    describe('getNetworks()', () => {
      it('should return array of networks', async () => {
        ...
        // Mock staticApiClient.get properly
