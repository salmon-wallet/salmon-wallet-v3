...
**Package:** `packages/ui` (shared component), `apps/mobile` (screen), `apps/extension` (consumer), `apps/web` (consumer), `packages/shared` (types)
...
**Package:** `packages/shared` (action already exists), `packages/ui` (shared component), `apps/mobile`, `apps/extension`, `apps/web`
...
The system SHALL allow the user to change the account avatar. This navigates to the existing `AccountAvatarPage` passing the account ID. Saving SHALL call `editAccount(targetId, { avatar })`.
...
**Package:** `packages/ui` (shared component), `apps/mobile`, `apps/extension`, `apps/web`
...
#### Scenario: All DOM platforms consume shared prop types
- **WHEN** extension and web implement AccountEditPage
- **THEN** both import the shared component from `@salmon/ui`
- **THEN** the shared component uses base prop types from `@salmon/shared`
- **WHEN** mobile implements AccountEditPage
- **THEN** it imports base prop types from `@salmon/shared` and extends them for React Native
